...
O-view was conceived after seeing [CodexBar](https://github.com/steipete/codexbar), a macOS menu-bar AI usage monitor. Working "next to" an existing open-source implementation creates a real risk that its code, structure, or naming leaks into the new project — whether deliberately or by osmosis from having read it.
...
2. **Technical correctness.** CodexBar is macOS/Swift. Its design encodes macOS assumptions — `NSStatusItem` text labels, Keychain, LaunchAgents, Safari cookie stores — that are **wrong on Windows** ([ADR-0003](0003-windows-tray-constraints.md)). Borrowing its structure would import defects, not shortcuts.

The second reason is the stronger one. The macOS design is not merely encumbered; it is inapplicable.

## Decision

O-view is developed **clean-room** with respect to CodexBar and any other existing usage-monitor implementation.

### Rules

1. **Do not read, clone, or browse CodexBar's source.** Not the repository, not individual files, not vendored copies.
...
4. **This rule binds AI coding assistants working on this repo**, which is why it is restated in `CLAUDE.md`. An assistant asked to "look at how CodexBar does X" must decline and reason from platform documentation instead.
...
| Action | Result |
|---|---|
| Fetched `github.com/steipete/codexbar` | **README-level description only** — features, platform, provider list. No source files read. |
| Fetched `codexbar.app` | HTTP 403, no content retrieved |
...
No CodexBar source code was accessed at any point.
